...
golgi_components.push(function load(){let t="productui-row",e=-1;customElements.define(t,class extends HTMLElement{constructor(){super(),e++,this.attachShadow({mode:"open"});this.shadowRoot.innerHTML=`<style>.td {
  display: table-cell;
}
:host {
  display: table-row;
}</style><div class="td" status="golgi:bind=status" style="golgi:bind=style">golgi:bind=name</div><div class="td">golgi:bind=price</div>`,this.name=t+"-"+e}onBeforeState(){this.observerStart()}observerCallback(t){"status"===t.attributeName&&"undefined"!==(t=t.target.getAttribute("status"))&&this[t]()}show(){this.style="display: '';"}hide(){this.style="display: none;"}})});
